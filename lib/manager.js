...
function Manager(parent='') { 
  ...
  this.createFile = async (fdata) => {
    ...
    const file = await fs.createWriteStream(pathname);
  ...
  this.listInfos = async (dirname='') => {
    const pathname = _correctPath(dirname);
    const filenames = await fs.readdirSync(pathname);
    const fileInfos = [];
  ...
  this.rename = async (filename, destFilename) => {
    ...
    setTimeout(async () => {
      await fs.renameSync(source, dest);
    }, 1000);
  ...
  this.cutPaste = async (filenames, destDir) => {
    if (await !fs.existsSync(destDir)) {
  ...
  this.touch = async (filenames) => {
    ...
    for (let i in filenames) {
      ...
      // file does not exist
      await fs.utimesSync(pathname, now, now);
    }
  ...
  this.mkdir = async (dirnames) => {
    ...
    for (let i in dirnames) {
      const dir = await this.createDir(dirnames[i]);
      crtDirs.push(dir);
    }
  ...
  async function _exists(pathname) {
    const exists = await fs.existsSync(pathname);
    return exists;
  }
}
